...
import Profile from '../../../pages/settings/my-account/profile';
type Props = {
...
const SettingNav = ({ children }) => {
  ...
  return (
    <nav className='no-scrollbar bg-muted bottom-0 left-0 top-0 z-20 flex max-h-screen h-screen static w-56 max-w-56 flex-col space-y-1 overflow-x-hidden overflow-y-scroll px-2 pb-3 transition-transform max-lg:z-10 lg:sticky lg:flex -translate-x-full opacity-0 lg:translate-x-0 lg:opacity-100'>
      <div>
      <button
        onClick={(e) => {
          e.preventDefault()
      ...
      <div>
        ...
        <Link href="/settings/my-account/profile"
          className={`font-medium leading-5 flex px-[11px] text-[0.875rem] ml-8 pt-[4px] justify-start h-[32px] w-[158px] mx-auto hover:bg-subtle rounded-md my-1 ${(router.pathname === "/settings/my-account/profile") ? "bg-emphasis" : ""}`}>
          Profile
        </Link>
      </div>
      {children}
